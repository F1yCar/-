...
async def capture_and_process_captions(x, y, width, height, last_sentences, last_translation, preprocess_options):
    try:
        ...
        text = pytesseract.image_to_string(frame, config=custom_config, lang='jpn')
        ...
        if text.strip():
            ...
            if new_original.strip():
                new_translation = await translate_text(new_original)
